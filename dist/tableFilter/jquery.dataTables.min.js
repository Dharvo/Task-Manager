...
$jscomp.polyfillIsolated = function (k, y, z, q) {
	...
	null != y &&
		(k
			...
			: y !== z &&
			  (($jscomp.propertyToPolyfillSymbol[G] =
					$jscomp.IS_SYMBOL_NATIVE
...
(function (k) {
	...
	function Ya(a) {
		...
		for (v = 0; v < l.length; v++) {
			var r = c[l[v]];
			null !== r.sWidth &&
				((r.sWidth = Zb(r.sWidthOrig, p)), (t = !0));
		}
	...
	function dc(a, b, c) {
		...
		b = function (h) {
			if (h && h.time) {
				var l = H(a, "aoStateLoadParams", "stateLoadParams", [
					a,
					h,
				]);
				if (
					...
					h.order !== q &&
						...
						k.each(h.order, function (n, m) {
							a.aaSorting.push(
								m[0] >= f.length ? [0, m[1]] : m
							);
						}));
					h.search !== q &&
						k.extend(a.oPreviousSearch, Wb(h.search));
					if (h.columns)
						for (d = 0, e = h.columns.length; d < e; d++)
							(l = h.columns[d]),
								l.visible !== q &&
									(f[d].bVisible = l.visible),
								l.search !== q &&
									k.extend(
										a.aoPreSearchCols[d],
										Wb(l.search)
	...
	var u = function (a) {
			...
			this.each(function () {
				...
				if ("table" != this.nodeName.toLowerCase())
					...
					if (null === f || "" === f)
						this.id = f =
							"DataTables_Table_" + u.ext._unique++;
					...
					g.aLengthMenu &&
						!g.iDisplayLength &&
						(g.iDisplayLength = Array.isArray(
							g.aLengthMenu[0]
						)
							? g.aLengthMenu[0][0]
							: g.aLengthMenu[0]);
	...
	var ub = function (a, b, c, d, e) {
			...
			for (h = b.length; n < h; n++) {
				...
				for (l = m.length; p < l; p++)
					(g = c(
						"string" === typeof m[p] ? m[p].trim() : m[p]
					)) &&
						g.length &&
						(f = f.concat(g));
			}
